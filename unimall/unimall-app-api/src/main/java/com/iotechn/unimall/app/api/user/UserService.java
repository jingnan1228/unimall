...
public interface UserService {
    ...
    public String register(
            @NotNull @TextFormat(length = 11)
            @HttpParam(name = "phone", type = HttpParamType.COMMON, description = "用户手机号") String phone,
            @NotNull @TextFormat(lengthMin = 6,lengthMax = 18,notChinese = true)
            @HttpParam(name = "password", type = HttpParamType.COMMON, description = "用户密码") String password, @HttpParam(name = "ip", type = HttpParamType.COMMON, description = "用户Ip") String ip,
//          @HttpParam(name = "ip", type = HttpParamType.IP, description = "用户Ip") String ip,
            @NotNull @HttpParam(name = "inviteCode", type = HttpParamType.COMMON, description = "上级邀请码"
            )Integer inviteCode
  ...
  public String register(
          ...
          @HttpParam(name = "ip", type = HttpParamType.IP, description = "用户Ip") String ip,
          @NotNull @HttpParam(name = "inviteCode", type = HttpParamType.COMMON, description = "上级邀请码")Integer inviteCode) throws ServiceException;
*/
    @HttpMethod(description = "用户绑定手机号")
    public String bindPhone(
            @NotNull @TextFormat(length = 11) @HttpParam(name = "phone", type = HttpParamType.COMMON, description = "用户手机号") String phone,
            @NotNull @TextFormat(lengthMin = 6,lengthMax = 18,notChinese = true)@HttpParam(name = "password", type = HttpParamType.COMMON, description = "设置用户密码") String password,
...
*/
    @HttpMethod(description = "查询余额和积分")
    public UserDTO selectBalanceAndIntegral(
            @NotNull @HttpParam(name = "userId", type = HttpParamType.COMMON, description = "用户ID") Long userId) throws Exception;
