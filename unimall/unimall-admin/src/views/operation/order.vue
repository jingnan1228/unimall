<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="refundDialogVisible" title="退款">
     ...
     <!-- 收款对话框 -->
    <el-dialog :visible.sync="payDialogVisible" title="确认收款">
      <el-form
        ref="refundForm"
        :model="refundForm"
        :rules="refundRules"
      ...
      >
        <el-form-item label="tips">
          <span>确认收款后改变订单状态，请确认您已经收到货款</span>
        </el-form-item>
        <el-form-item label="操作">
          <el-radio v-model="refundForm.type" label="0">未收款</el-radio>
          <el-radio v-model="refundForm.type" label="1">确认收款</el-radio>
        </el-form-item>
        <el-form-item v-if="refundForm.type === 1 || refundForm.type === '1'" label="金额">
          <el-input v-model="refundForm.sum"/>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :disabled="refundSubmiting" type="primary" @click="confirmPay">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import { listOrder, shipOrder, refundOrder,payOrder, detailOrder, getExcelInfo, editAdminMono, getExcelStatistics } from '@/api/order'
      ...
      this.$refs['refundForm'].validate(valid => {
        if (valid) {
          ...
          const obj = { 
           ...
           if (this.refundForm.sum) {
            obj['payPrice'] = parseInt(this.refundForm.sum * 100)
          }
          payOrder(obj)
            .then(response => {
              this.refundSubmiting = false
