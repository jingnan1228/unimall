    ...
    public String groupShopStatusRefund(String orderNo) throws ServiceException {
        if (lockComponent.tryLock(OrderBizService.ORDER_REFUND_LOCK + orderNo, 30)) {
            try {
                ...
                wxPayRefundRequest.setTotalFee(orderDO.getPayPrice().intValue() - orderDO.getFreightPrice().intValue());
                wxPayRefundRequest.setRefundFee(orderDO.getPayPrice().intValue() - orderDO.getFreightPrice().intValue());
