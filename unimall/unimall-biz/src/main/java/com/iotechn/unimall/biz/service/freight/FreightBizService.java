...
public class FreightBizService {
    ...
    public Integer getFreightMoney(OrderRequestDTO orderRequestDTO) {
        ...
        Integer discount = orderRequestDTO.getCoupon() != null ? orderRequestDTO.getCoupon().getDiscount().intValue() : 0;
    ...
    private Integer getMoney(FreightTemplateDTO freightTemplateDTO, Integer num, String province, Integer moneySubDiscount) {
        ...
        for (FreightTemplateCarriageDO freightTemplateCarriageDO : freightTemplateCarriageDOList) {
            ...
            if (freightTemplateCarriageDO.getDesignatedArea().contains(province)) {
                //如果包邮返回0
                if (freightTemplateCarriageDO.getFreePrice().intValue() == 0) {
                    return 0;
                }
                //必须为正数才是满邮活动
                if (moneySubDiscount >= freightTemplateCarriageDO.getFreePrice().intValue() && freightTemplateCarriageDO.getFreePrice().intValue() > 0) {
                    return 0;
                }

                Integer open = freightTemplateCarriageDO.getFirstMoney().intValue();
                ...
                if (num % freightTemplateCarriageDO.getContinueNum() != 0) {
                    open = open + freightTemplateCarriageDO.getContinueMoney().intValue() * ((num / freightTemplateCarriageDO.getContinueNum().intValue()) + 1);
                } else {
                    open = open + freightTemplateCarriageDO.getContinueMoney().intValue() * (num / freightTemplateCarriageDO.getContinueNum().intValue());
                }
                return open;
            }
        }
        ...
        if (freightTemplateDO.getDefaultFreePrice().intValue() == 0) {
            return 0;
        }
        if (freightTemplateDO.getDefaultFreePrice().intValue() > 0 && moneySubDiscount >= freightTemplateDO.getDefaultFreePrice().intValue()) {
            return 0;
        }

        Integer open = freightTemplateDO.getDefaultFirstMoney().intValue();
        ...
        if (num % freightTemplateDO.getDefaultContinueNum() != 0) {
            open = open + freightTemplateDO.getDefaultContinueMoney().intValue() * ((num / freightTemplateDO.getDefaultContinueNum().intValue()) + 1);
        } else {
            open = open + freightTemplateDO.getDefaultContinueMoney().intValue() * (num / freightTemplateDO.getDefaultContinueNum().intValue());
        }
        return open;
    }
